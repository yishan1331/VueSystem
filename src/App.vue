  ...
  created: function() {
    ...
    if (vm.$router.currentRoute.path != "/login") {
      ...
      if (
        ...
        obj.accesslist = JSON.parse(sessionStorage.getItem("loginAccessList"));
        var access = JSON.parse(sessionStorage.getItem("loginAccessList"));
    ...
    window.addEventListener("beforeunload", () => {
      if (vm.loginData.status === true) {
        sessionStorage.setItem("loginStatus", Boolean(vm.loginData.status));
