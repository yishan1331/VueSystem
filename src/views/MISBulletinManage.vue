<template>
  <div class="MISBulletinManage container">
            ...
            <b-row>
              <b-col lg="12" class="pb-2">
                <div class="text-center">
                  <b-button type="submit" variant="primary" @click.prevent="onSubmit()">上傳</b-button>
                </div>
              </b-col>
            </b-row>
          </b-form>
    ...
    <!-- 公告修改modal -->
    <b-modal centered v-model="modBulletinModalShow" size="xl" no-close-on-backdrop no-close-on-esc>
    ...
    queryResponse: {
      handler() {
        ...
        for (var i = 0; i < vm.queryResponse.length; i++) {
          ...
          itemsobj["category"] = categorytoCH[vm.queryResponse[i]["category"]];
  ...
  methods: {
    ...
    onModify() {
      ...
      if (Object.keys(vm.modmodalcontent.boardannex).length == 0) {
        ...
        formData.append("filename", Object.keys(vm.modmodalcontent.boardannex));
      }
    ...
    onDelete() {
      ...
      axios
        .post("/static/php/views/misbulletin/misBulletinDel.php", params)
        .then(
          function(response) {
            const result = response.data;
            vm.setalertMsg(result);
