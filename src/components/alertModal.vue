        ...
        >
            <template v-slot:default>
                <template v-if="alertProgressModalShow">
                    <h4>進度：</h4>
                    <b-progress :max="100" variant="success" height="1.5rem" show-progress animated>
                        <b-progress-bar :value="alertProgressValue">
                            <p
                                style="margin: 0;font-size: 1.2rem;font-weight: bolder;"
                            >{{ alertProgressValue }}%</p>
                        </b-progress-bar>
                    </b-progress>
                </template>
                <template v-else>
                    <div class="text-center" v-if="typeof(alertMsg) === 'string'">
                        <b-spinner
                            v-if="alertMsg === '請稍候....'"
                            ...
                            type="grow"
                        ></b-spinner>
                        <h4 :class="{'inline-block':alertMsg === '請稍候....'}">{{ alertMsg }}</h4>
                    </div>
                    <h4
                        v-else
