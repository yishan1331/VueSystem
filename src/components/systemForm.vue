<template>
  <div class="systemForm">
    <b-form>
      <b-row class="my-4" v-for="(item1,key1) in vforData" :key="key1" v-if="key1 != 'button'">
        <b-col sm="2">
      ...
      <b-row v-else>
        <b-col lg="12" class="pb-2">
          <div class="text-center">
            <b-button type="submit" variant="primary" @click.prevent="submit()">{{item1[1]}}</b-button>
          </div>
        </b-col>
      </b-row>
    </b-form>
...
export default {
  ...
  methods: {
      ...
      for (var i = 0; i < Object.keys(formdata).length; i++) {
        if (Object.values(formdata)[i].length == 2) {
          ...
          if (Object.values(formdata)[i][0] == "accesscheckbox") {
            ...
            formdataitem[Object.keys(formdata)[i]] = Object.values(formdata)[
              i
            ][2];
          }
        }
    ...
    submit() {
      var vm = this;
      console.log(this.vforData.button);
